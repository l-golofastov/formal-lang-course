...
from project.context_free.matrix import *
...
def test_cfpq_matrix_1():
    ...
    assert cfpq(grammar, graph, {0, 2}, {2, 4}, Variable("S"), matrix) == expected
...
def test_cfpq_matrix_2():
    ...
    assert cfpq(grammar, graph, {0, 1}, {1, 2, 3}, algorithm=matrix) == {
        (0, 2),
        (1, 3),
        (1, 2),
        (0, 3),
...
def test_cfpq_matrix_3():
    ...
    assert cfpq(grammar, graph, {0}, {0, 1}, Variable("S"), matrix) == {
        (0, 0),
        (0, 1),
    }


def test_cfpq_matrix_4():
    grammar = CFG.from_text(
        """
        S -> A B
        S -> A C
    ...
    assert cfpq(grammar, graph, None, None, Variable("S"), matrix) == {
        (3, 4),
        (2, 5),
        (1, 0),
        (0, 4),
...
def test_cfpq_matrix_5():
    ...
    assert cfpq(grammar, graph, {0}, {1, 2, 4}, Variable("S"), matrix) == set()
